      ...
      <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            {(SITE.addressLines.length > 0 || SITE.phoneDisplay || SITE.email) && (
              <address className="mt-6 space-y-1.5 text-sm not-italic leading-relaxed text-porcelain/70">
                {SITE.addressLines.length > 0 && (
                  <p>{SITE.addressLines.join(", ")}</p>
                )}
                {SITE.phoneDisplay && (
                  <p>
                    <a href={`tel:${SITE.phoneE164}`} className="transition-colors hover:text-brass">
                      {SITE.phoneDisplay}
                    </a>
                  </p>
                )}
          ...
          <nav className="lg:col-span-3" aria-label="Explore">
            <p className="eyebrow text-porcelain/65">Explore</p>
          ...
          <nav className="lg:col-span-2" aria-label="Work with us">
            <p className="eyebrow text-porcelain/65">Work With Us</p>
            <ul className="mt-4 space-y-2.5">
              {workWithUs.map((l) => (
                <li key={l.to}>
                  <Link to={l.to} className="text-sm text-porcelain/80 transition-colors hover:text-brass">
          ...
          <nav className="lg:col-span-3" aria-label="Healthcare network">
            <p className="eyebrow text-porcelain/65">The Network</p>
            <ul className="mt-4 space-y-2.5">
        ...
        <div className="mt-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <p className="font-mono text-[0.6875rem] uppercase tracking-eyebrow text-porcelain/60">
              ...
              {SITE.socials.map((s) => (
                <li key={s.href}>
                  <a
                    ...
                    aria-label={`Zendale Limited on ${s.label}`}
                    className="flex h-9 w-9 items-center justify-center border border-porcelain/20 text-porcelain/70 transition-colors hover:border-brass hover:text-brass"
                  >
