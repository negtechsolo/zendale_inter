  ...
  return (
    <div>
      <label htmlFor={id} className={`block text-sm font-medium ${dark ? "text-porcelain" : "text-ink"}`}>
        {label}
        {optional && <span className={`ml-2 font-normal ${dark ? "text-porcelain/65" : "text-carbon/70"}`}>(optional)</span>}
      </label>
...
const inputCls = (dark?: boolean, error?: string) =>
  `w-full border px-4 py-3 text-sm focus:outline-none ${
    dark
      ? "border-porcelain/25 bg-transparent text-porcelain placeholder:text-porcelain/60 focus:border-brass"
      : "border-ink/20 bg-white text-carbon placeholder:text-carbon/70 focus:border-steel"
  } ${error ? "border-[#A2542F]" : ""}`;
