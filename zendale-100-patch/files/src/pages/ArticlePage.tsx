...
import { Seo, addJsonLd } from "../lib/seo";
...
function buildFaqSchema(article: Article) {
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
 ...
 *
 * On the server the two blocks are handed to the pre-renderer so they ship
 * inside the static HTML, which is what search engines and answer engines
 * read. In the browser they are injected on navigation, replacing any
 * pre-rendered copy so the page never carries two versions of the same graph.
 */
function ArticleSchema({ article }: { article: Article }) {
  if (typeof window === "undefined") {
    addJsonLd(buildArticleSchema(article));
    addJsonLd(buildFaqSchema(article));
  }

  useEffect(() => {
    document
      .querySelectorAll("script[data-zendale-article-schema], script[data-zendale-faq-schema]")
      .forEach((node) => node.remove());

    const script = document.createElement("script");
    script.type = "application/ld+json";
    script.dataset.zendaleArticleSchema = article.slug;
    script.text = JSON.stringify(buildArticleSchema(article));
    document.head.appendChild(script);

    const faqScript = document.createElement("script");
    faqScript.type = "application/ld+json";
    faqScript.dataset.zendaleFaqSchema = article.slug;
    faqScript.text = JSON.stringify(buildFaqSchema(article));
        ...
        <div className="bg-porcelain py-16 lg:py-24">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-12 lg:px-8">
            ...
            <div className="lg:col-span-8 lg:col-start-5">
                ...
                <ul className="mt-5 space-y-3 text-sm leading-relaxed text-carbon/80">
                  {article.sources.map((source) => (
                    <li key={source.url}>
                      ...
                      <span className="text-carbon/70"> · {source.organisation}</span>
                    </li>
                  ))}
                </ul>
              </Reveal>
              ...
              <p className="mt-8 text-xs leading-relaxed text-carbon/70">
